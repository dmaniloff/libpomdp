...
%% play the pomdp
logFilename = sprintf('simulation-logs/rocksample/RS710-HYVI-regions-always-root-ADD-%s.log', datestr(now, 'yyyy-mmm-dd-HHMMSS'));
diary(logFilename);
...
EXPANSION_RATE       = 35; % calculated from the online simulator, avg
AVG_EP_LIFETIME      = 27; % calculated from the online simulator, avg
EPSILON_ACT_TH       = 1e-3;
EPISODECOUNT         = 5;
...
for run = 1:TOTALRUNS
    ...
    for ep = 1:EPISODECOUNT
        ...
        for iter = 1:MAXEPISODELENGTH
            ...
            %
            % will just always backup at the root node
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            aoTree.backupLowerAtNode(rootNode);
...
statsFilename = ...
    sprintf('simulation-logs/rocksample/RS710-ALLSTATS-HYVI-regions-always-root-ADD-%s.mat', datestr(now, 'yyyy-mmm-dd-HHMMSS'));
