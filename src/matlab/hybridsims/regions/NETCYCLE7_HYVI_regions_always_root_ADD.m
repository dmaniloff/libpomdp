...
logFilename = sprintf('simulation-logs/network/NETCYCLE%d-HYVI-regions-always-root-ADD-%s.log',...
                      NUM_MACHINES, datestr(now, 'yyyy-mmm-dd-HHMMSS'));
diary(logFilename);

% parameters
EXPANSION_RATE       = 55; % calculated from the online simulator, avg
AVG_EP_LIFETIME      = 50;
EPSILON_ACT_TH       = 1e-3;
...
statsFilename = ...
    sprintf('simulation-logs/network/NETCYCLE%d-ALLSTATS-HYVI-regions-always-root-ADD-%s.mat',...
            NUM_MACHINES, datestr(now, 'yyyy-mmm-dd-HHMMSS'));
