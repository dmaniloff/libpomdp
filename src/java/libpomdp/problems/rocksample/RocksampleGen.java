...
public class RocksampleGen {
	...
	for (c = 0; c < l; c++) {
		...
		s = c;
	    v = v.concat(indent(ind) + "(" + varname + c + " (" + varname + "'"
		    + massX(varname, l, s) + "))\n");
	}
	...
	for (c = 0; c < l; c++) {
		...
		v = v.concat(indent(ind) + "(" + varname + "" + c + " ("
			+ ILLEGAL_PENALTY + "))\n");
	}
	...
	for (c = 0; c < n + 1; c++) {
		...
		if (c == k[r][0] && d == k[r][1]) {
		    v = v.concat("\n");
		    v = v.concat(indent(ind + 6) + "(r" + r + " (g (r" + r
			    + "' (g (0.0)) (b (1.0))))" + "\n"
			    + indent(ind + 9) + " (b (r" + r
			    + "' (g (0.0)) (b (1.0))))))" + "\n");
		} else {
	...
	for (c = 0; c < n + 1; c++) {
	    v = v.concat(indent(ind + 1) + "(j" + c + "\n");
	    // check for the terminal state, give uniform obs here - does it
	    // matter?
	    if (c == n) {
		...
		v = v.concat(indent(ind + 6) + "(r" + r + "' (g (o' (og ("
			+ acc(c, d, r) + ")) (ob (" + (1 - acc(c, d, r))
			+ "))))" + "\n" + indent(ind + 10) + " (b (o' (og ("
			+ (1 - acc(c, d, r)) + ")) (ob (" + acc(c, d, r)
			+ "))))))" + "\n");
		// }else {
		// v=v.concat(" (SAMEr"+r+"))\n");
		// }
	...
	double dist = Math.sqrt(Math.pow(j - k[r][0], 2)
		+ Math.pow(i - k[r][1], 2));
