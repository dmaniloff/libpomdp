...
public class OmniWumpus implements Wumpus {
	...
	// south of the wumpus, the wumpus will move north with probability ol /
	// 2
	if (ay < wy) {
	    ret[1] = ol / 2.0;
	    ret[3] = 0d;
	}
	// north of the wumpus, the wumpus will move south with probability ol /
	// 2
	else if (ay > wy) {
	    ret[1] = 0d;
	    ret[3] = ol / 2.0;
	}
	// same row than the wumpus, the wumpus will move north or south with
	// probability ol / 4
	else if (ay == wy) {
	    ret[1] = ol / 4.0;
	    ret[3] = ol / 4.0;
	...
	// west of the opponent, the wumpus will move east with probability ol /
	// 2
	else if (ax < wx) {
	    ret[2] = ol / 2.0;
	    ret[4] = 0d;
	}
	// same column of the wumpus, the wumpus will move west or east with
	// probability ol / 4
	else if (ax == wx) {
	    ret[2] = ol / 4.0;
	    ret[4] = ol / 4.0;
	...
	if (gp.wallAhead(wpos, CatchGen.Direction.N)) {
	    ret[0] += ret[1];
	    ret[1] = 0d;
	}
	if (gp.wallAhead(wpos, CatchGen.Direction.S)) {
	    ret[0] += ret[3];
	    ret[3] = 0d;
	}
	if (gp.wallAhead(wpos, CatchGen.Direction.E)) {
	    ret[0] += ret[2];
	    ret[2] = 0d;
	}
	if (gp.wallAhead(wpos, CatchGen.Direction.W)) {
	    ret[0] += ret[4];
	    ret[4] = 0d;
	}
