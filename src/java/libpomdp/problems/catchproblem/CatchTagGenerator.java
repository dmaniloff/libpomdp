 ...
 * libpomdp
 * ========
 * File: CatchTagGenerator.java
 * Description: the reward here is obtained from a tagging action, there is
 *              also a sink state, the tagged state, so a single tag is the
 *              only objective
 *              indentation is 2
...
public class CatchTagGenerator {
    ...
    public CatchTagGenerator(int width, int height, CatchGridProperties gp,
	    Wumpus wumpus, Sensor s, PrintStream out) {
	this.out = out;
	this.gp = gp;
	this.w = wumpus;
	this.s = s;
	...
	out.println("// ========");
	out.println("// File: autogenerated by CatchTagGenerator.java");
	...
	// tagged state
	// tagged0 = not tagged; tagged1 = tagged
	out.println(ind(2) + varDecl("tagged", 2));
	out.println(")");
	...
	// sensing of the wumpus - this dictates the kind of sensing our agent
	// has
	// we call it coll since originally this indicated collocation with the
	// wumpus
	...
	out.println("init [*" + buildInit()
		+ "(tagged (tagged0 (1.0)) (tagged1 (0.0))) ]");

	// some CPTS may be unnormalized -
	// this means all CPTs will be automatically normalized when read in
	out.println("// some CPTS may be unnormalized");
	out.println("// -----------------------------");
	out.println("unnormalised");
	...
	out.println(ind(2) + "apos (SAMEapos)");
	out.println(ind(2) + "wpos (SAMEwpos)");
	out.println(ind(2) + "tagged " + taggedState());
	out.println(ind(2) + "observe");
	out.println(ind(2) + "aloc (ldd)");
	out.println(ind(4) + "coll (sensingdd)");
	out.println(ind(2) + "endobserve");
	out.println("cost " + tagCost());
	out.println("endaction");
	out.println();

	// north
	out.println("action north");
	out.println(ind(2) + "apos " + move(CatchGen.Direction.N));
	out.println(ind(2) + "wpos (wumpusb)");
	out.println(ind(2) + "tagged (SAMEtagged)");
	out.println(ind(2) + "observe");
	out.println(ind(2) + "aloc (ldd)");
	out.println(ind(4) + "coll (sensingdd)");
	out.println(ind(2) + "endobserve");
	out.println("cost (tagged (tagged0 (1.0)) (tagged1 (0.0)))");
	out.println("endaction");
	out.println();

	// south
	out.println("action south");
	out.println(ind(2) + "apos " + move(CatchGen.Direction.S));
	out.println(ind(2) + "wpos (wumpusb)");
	out.println(ind(2) + "tagged (SAMEtagged)");
	out.println(ind(2) + "observe");
	out.println(ind(2) + "aloc (ldd)");
	...
	out.println(ind(2) + "apos " + move(CatchGen.Direction.E));
	out.println(ind(2) + "wpos (wumpusb)");
	out.println(ind(2) + "tagged (SAMEtagged)");
	out.println(ind(2) + "observe");
	out.println(ind(2) + "aloc (ldd)");
	...
	out.println(ind(2) + "apos " + move(CatchGen.Direction.W));
	out.println(ind(2) + "wpos (wumpusb)");
	out.println(ind(2) + "tagged (SAMEtagged)");
	out.println(ind(2) + "observe");
	out.println(ind(2) + "aloc (ldd)");
	...
	for (int i = 0; i < N; i++) {
		...
		if (!gp.isLegalPosition(i) || !gp.isLegalPosition(j)
			|| (i == j)) {
		    v = v.concat(ind(4) + "(wpos" + j + " (0.0))\n");
		} else {
		    // last case - sum up to 1.0
		    if (cases == totcases - 1) {
			v = v.concat(String.format(ind(4) + "(wpos" + j
				+ " (%.14f))\n",
    ...
    // move actions
    // private String move(CatchGen.Direction m) {
    // String v = "(tagged (tagged0 (apos\n";
	...
	for (int currPos = 0; currPos < N; currPos++) {
		...
		v = v.concat(ind(4) + "(apos" + currPos + " (apos'\n");
		v = v.concat(ind(6)
			+ massX("apos", N, getAdjacentPos(currPos, m)));
		v = v.concat(ind(4) + "))\n");
	    }
	}
	v = v.concat(ind(2) + ")\n");
    ...
    private int getAdjacentPos(int p1, CatchGen.Direction move) {
	int x1 = gp.getxy(p1)[0];
	int y1 = gp.getxy(p1)[1];
	switch (move) {
	case N:
	...
	for (int c = 0; c < l; c++) {
		...
		// last case
		v = v.concat(String.format("(" + varname + c + " (%.14f))",
			(1.0 - prob * (cases - 1.0))));
		cs++;
	    } else {
		v = v.concat(String.format("(" + varname + c + " (%.14f))",
    ...
    // when the absolute positions are equal, (coll' (coll0 (0.0)) (coll1
    // (1.0)))
	...
	for (int c = 0; c < N; c++) {
	    ...
	    for (int k = 0; k < N; k++) {
		// see if the wumpus is in range according to the sensing
		// function...
		if (s.inRange(c, k))
		    v = v.concat(ind(4) + "(wpos" + k
			    + " (coll' (coll0 (0.0)) (coll1 (1.0))))\n");
		else
		    v = v.concat(ind(4) + "(wpos" + k
			    + " (coll' (coll0 (1.0)) (coll1 (0.0))))\n");
	    }
	    v = v.concat(ind(2) + "))\n");
	}
	v = v.concat(")\n");
    ...
    // cost (reward) for tagging when collocated
    private String tagCost() {
	String v = "(apos\n";
	for (int c = 0; c < N; c++) {
	    v = v.concat(ind(2) + "(apos" + c + " (wpos\n");
	    for (int k = 0; k < N; k++) {
		// see if the wumpus is in our square
		if (c == k)
		    v = v.concat(ind(4) + "(wpos" + k
			    + " (tagged (tagged0 (-10)) (tagged1 (0.0))))\n");
		else
		    v = v.concat(ind(4) + "(wpos" + k
			    + " (tagged (tagged0 ( 10)) (tagged1 (0.0))))\n");
	    }
    ...
    // move into the tagged state after executing the catch action
    private String taggedState() {
	String v = "(apos\n";
	for (int c = 0; c < N; c++) {
	    v = v.concat(ind(12) + "(apos" + c + " (wpos\n");
	    for (int k = 0; k < N; k++) {
		// see if the wumpus is in our square
		if (c == k)
		    v = v.concat(ind(14) + "(wpos" + k
			    + " (tagged' (tagged0 (0.0)) (tagged1 (1.0))))\n");
		else
		    v = v.concat(ind(14) + "(wpos" + k + " (SAMEtagged))\n");
		// v=v.concat(ind(14)+ "(wpos"+k+
		// " (tagged' (tagged0 (0.0)) (tagged1 (1.0))))\n");
	    }
	    v = v.concat(ind(12) + "))\n");
	...
	for (int c = 0; c < N; c++) {
		...
		actd = w.getActDist(c, k);
		for (int s = 0; s < N; s++) {
		    // this assumes the wumpus can only move to its adjacent
		    // cells...
		    if (getAdjIndex(k, s) >= 0)
			v = v.concat("(wpos"
				+ s
