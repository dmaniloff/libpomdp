...
public class AndOrTreeUpdateAdd extends AndOrTree {
     ...
     * 
     * one-step expansion of |A||O| HybridValueIterationOrNodes fully overridden
     * here to access internals and have more control - allows for greater speed
     */
    public void expand(HybridValueIterationOrNode en) {
	// make sure this node hasn't been expanded before
	if (en.getChildren() != null) {
	    System.err
		    .println("node cannot be expanded, it already has children");
	    return;
	}
	...
	for (int action = 0; action < problem.nrActions(); action++) {
	    ...
	    a.init(action, en, problem.expectedImmediateReward(
	    ...
	    // sample subset of the children based on pOba
	    Set<Integer> sampledChildren = Utils.sampleSubset(pOba.getData());
	    // nullify pOba everywhere exept where we sampled
	    pOba = Utils.setZeroNonSampled(pOba, sampledChildren);
	    
	    a.initChildren(problem.nrObservations(), pOba);
	    // iterate through new fringe OR nodes
	    for (Integer observation : sampledChildren) {
		// type-cast, doon't yet know of a nicer way to do this
		o = a.getChild(observation);
		// ZERO-PROB OBSERVATIONS:
		// here we should continue the loop and avoid re-computing V^L
		// and V^U
		...
		o.init(problem.nextBeliefState(en.getBeliefState(), action,
			observation), observation, a);
		o.getBeliefState().setPoba(pOba.get(observation));
		// compute upper and lower bounds for this node
		o.u = offlineUpper.value(o.getBeliefState());
		o.l = offlineLower.value(o.getBeliefState());
	    ...
	    a.bakCandidate = new HybridValueIterationOrNode[offlineLower.size()]; // all
										  // nulls
	} // andNode loop
     ...
     * updateAncestors:
     * 
     * now keeps track of the best candidate node to backup maintains |A| fringe
     * lists
     */
	...
	while (n != root) { // reference comparison
	    ...
	    o = a.getParent();
	    // update the HybridValueIterationOrNode that is parent of the
	    // parent
	    o.l = ORpropagateL(o);
	    o.u = ORpropagateU(o);
	    // H(b,a)
     ...
     * ORpropagateLexpand:
     * 
     * Need a special propagate function for the expand case of an
     * HybridValueIterationOrNode since we are now saving the oneStepBestAction,
     * but we cannot overwrite this during updateAncestors since it is with
     * respect to the current lower bound.
     * 
     * @param o
     * @return
     * 
	...
	for (OrNode o : root.getChild(bestA).getChildren()) {
	    ...
	    // problem.gao(lowerBound[o.belief.getplanid()], bestA,
	    // o.getobs()).display();
	    gab = OP.add(gab, problem.gao(lowerBound[o.getBeliefState()
		    .getAlphaVectorIndex()], bestA, o.getObs()));
	}
     ...
     * backupLowerAtNode:
     * 
     * backup the lower bound at the given HybridValueIterationOrNode and update
     * the offline lower bound by adding the new alpha vector to the value
     * function representation Using the current info from the tree, a backup
     * operation is reduced to computing a particular gab vector
     */
	...
	DD gamma = DDleaf.myNew(problem.getGamma());
	DD gab = DD.zero;
	// int bestA = currentBestActionAtNode(on); // consider caching this
	// value maybe
	int obs = 0;
	DD lowerBound[] = ((ValueFunctionAdd) offlineLower).getvAdd();
	// \sum_o g_{a,o}^i
	for (OrNode o : on.getChild(on.oneStepBestAction).getChildren()) {
	    if (o == null) {
		// in this case we use any valid supporting alpha to compute
		// g_{a,o}^{i}
		gab = OP.add(
			gab,
			problem.gao(
				lowerBound[on.getChild(on.oneStepBestAction).validPlanid],
				on.oneStepBestAction, obs));
	    } else {
		// compute g_{a,o}^{planid}
		gab = OP.add(gab, problem.gao(lowerBound[o.getBeliefState()
			.getAlphaVectorIndex()], on.oneStepBestAction, obs));
	    }
	    // iterate counter
	    obs++;
	}
    ...
    /**
     * expectedReuse: calculate expected # of belief nodes to reuse given
     * current expanded tree and best action to execute
     */
	...
	for (OrNode o : root.getChild(bestA).getChildren()) {
	    ...
	    if (o != null)
		expR += o.getBeliefState().getPoba() * o.getSubTreeSize();
	}
	return expR;
    } // expectedReuse
	...
	return expectedReuse()
		/ (root.getSubTreeSize() - problem.nrObservations()
			* problem.nrActions());
    } // expectedReuseRatio
	...
	out.format(o.hashCode() + "[label=\""
		...
		"U(b)= %.2f\\n" + "L(b)= %.2f\\n" + "expH(b)= %.2f\\n"
		+ "expH*(b)= %.2f\\n" + "bakH(b)= %.2f" + "\"];\n", o.u, o.l,
		o.h_b, o.hStar, o.bakHeuristic);
	// every or node has a reference to the best node to expand in its
	// subtree
	out.println(o.hashCode() + "->" + o.bStar.hashCode()
		+ "[label=\"b*\",weight=0,color=blue];");
	// if this is the root, then print an edge to the best candidate node to
	// backup
	if (o == getRoot()) {
	...
	for (AndNode a : o.getChildren()) {
	    out.print(o.hashCode() + "->" + a.hashCode() + "[label=\""
	...
	out.format(
		a.hashCode() + "[label=\"" + "a="
			+ problem.getActionString(a.getAct()) + "\\n"
			+ "U(b,a)= %.2f\\n" + "L(b,a)= %.2f" + "\"];\n", a.u,
		a.l);
	...
	for (HybridValueIterationOrNode o : a.getChildren()) {
	    if (!(o == null))
		out.format(a.hashCode() + "->" + o.hashCode() + "[label=\""
			+ "obs: " + problem.getObservationString(o.getObs())
			+ "\\n" + "P(o|b,a)= %.2f\\n" + "H(b,a,o)= %.2f"
			+ "\"];", o.getBeliefState().getPoba(), o.h_bao);
	}
	out.println();

	// every or node has a reference to the best node to backup in its
	// subtree
